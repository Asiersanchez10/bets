...
public class CreateEventGUI extends JFrame {
	...
	private TrabajadorGUI TrabajadorAnterior;
	private final JLabel lblNewLabel = new JLabel(ResourceBundle.getBundle("Etiquetas").getString("Description")); //$NON-NLS-1$ //$NON-NLS-2$
	
	public CreateEventGUI(TrabajadorGUI trabajadorAnterior) {
		try {
			this.TrabajadorAnterior=trabajadorAnterior;
			CreateEventGUIEjecucion();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void CreateEventGUIEjecucion() throws Exception {
	...
	private void jButtonClose_actionPerformed(ActionEvent e) {
		this.TrabajadorAnterior.setVisible(true);
